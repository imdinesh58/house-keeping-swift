...
class tblAssignTasksPending: UIViewController, UITableViewDelegate, UITableViewDataSource, UIGestureRecognizerDelegate  {
    @IBOutlet weak var tblAssignTasksPending: UITableView!
    let defaults = UserDefaults.standard
    override func viewDidLoad() {
        super.viewDidLoad()
        print("View controller Pending loaded")
        self.tblAssignTasksPending?.delegate = self
        self.tblAssignTasksPending?.dataSource = self
        ...
        print("if objects.isEmpty == at viewdidload  pending", objects.isEmpty)
        getListJSON()
        GETFloorDetails()
        ...
        if #available(iOS 10.0, *) {
            ...
            refreshControl.addTarget(self,
                                     action: #selector(refreshPOptions(sender:)),
                                     for: .valueChanged)
            self.tblAssignTasksPending.refreshControl = refreshControl
        }
    }
    
    @objc private func refreshPOptions(sender: UIRefreshControl) {
        objects.removeAll()
        objects2.removeAll()
        objects3.removeAll()
        objects4.removeAll()
        ...
        if EmpArr.count == 0 {
            for object in objects.reversed() {
                ...
                if object.taskStatusID == "2" {
                    ...
                    self.tblAssignTasksPending?.reloadData()
                }
            }
        }
        print("After Pending" ,EmpArr.count)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = self.tblAssignTasksPending.dequeueReusableCell(withIdentifier: "tasksPending", for: indexPath) as! AllTasksCell
        cell.lbl1?.text = ""
        cell.lbl2?.text = ""
        cell.lbl3?.text = ""
        cell.lbl4?.text = ""
