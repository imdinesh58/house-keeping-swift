...
class tblAssignTasksCompleted: UIViewController, UITableViewDelegate, UITableViewDataSource, UIGestureRecognizerDelegate {
    @IBOutlet weak var tblAssignTasksCompleted: UITableView!

    let defaults = UserDefaults.standard
    
    override func viewDidLoad() {
        super.viewDidLoad()
        print("View controller COMPLETED loaded")
        SwiftSpinner.show("Loading")
        self.tblAssignTasksCompleted?.delegate = self
        self.tblAssignTasksCompleted?.dataSource = self
        objects.removeAll()
        objects2.removeAll()
        objects3.removeAll()
        objects4.removeAll()
       ...
       // print("if objects.isEmpty == at viewdidload  completed ", objects.isEmpty)
        getListJSON()
        GETFloorDetails()
        ...
        if #available(iOS 10.0, *) {
            ...
            refreshControl.addTarget(self,
                                     ...
                                     for: .valueChanged)
            self.tblAssignTasksCompleted.refreshControl = refreshControl
        }
    }
        ...
        if AssignedTaskIdArr.count == 0 {
            for object in objects.reversed() {
                ...
                if object.taskStatusID == "3" {
                    ...
                    self.tblAssignTasksCompleted?.reloadData()
                }
            }
        }
        print("After completed" ,AssignedTaskIdArr.count)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "tasksCompleted", for: indexPath) as! AllTasksCell
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        self.ViewTaskDescription(pendingText : send! , pendingText2 : send2! , pendingText3 : send3! , pendingText4 : send4! , pendingText5 : send5! , pendingText6 : send6! , pendingText7 : send7!, pendingText8 : send8!, pendingText9 : send9!)
    }
    ...
    func ViewTaskDescription(pendingText: String, pendingText2: String, pendingText3: String, pendingText4: String ,pendingText5: String, pendingText6: String, pendingText7: String,  pendingText8: String, pendingText9: String) {
