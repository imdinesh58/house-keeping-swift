...
class tblDoNotDistrub: UIViewController, UITableViewDelegate, UITableViewDataSource, UIGestureRecognizerDelegate  {
    @IBOutlet weak var tblDND: UITableView!
    let defaults = UserDefaults.standard
    override func viewDidLoad() {
        super.viewDidLoad()
        print("View controller DND loaded")
        self.tblDND?.delegate = self
        self.tblDND?.dataSource = self
        ...
        print("if objects.isEmpty == at viewdidload  DND ", objects.isEmpty)
        getListJSON()
        GETFloorDetails()
        ...
        if #available(iOS 10.0, *) {
            ...
            refreshControl.addTarget(self,
                                     action: #selector(refreshDOptions(sender:)),
                                     for: .valueChanged)
            self.tblDND.refreshControl = refreshControl
        }
    }
    
    @objc private func refreshDOptions(sender: UIRefreshControl) {
        objects.removeAll()
        objects2.removeAll()
        objects3.removeAll()
        objects4.removeAll()
    ...
    func getAssignTasksDND() {
        ...
        for object in objects.reversed() {
            ...
            if object.taskStatusID == "4" {
                ...
                self.taskListIdArr.append(object.checkListIds)
                self.priorityArr.append(object.priorities)
                self.AssignedByArr.append(object.assignedByName)
                self.tblDND?.reloadData()
            }
          }
        }
        print("After DND" ,EmpArr.count)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = self.tblDND.dequeueReusableCell(withIdentifier: "tasksDND", for: indexPath) as! AllTasksCell
        cell.lbl1?.text = ""
        cell.lbl2?.text = ""
        cell.lbl3?.text = ""
        cell.lbl4?.text = ""
       ...
       // print("self.taskListIdArr[indexPath.row]  ", self.taskListIdArr[indexPath.row])
