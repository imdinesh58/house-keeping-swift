    ...
    class tblHistoryPending: UIViewController, UITableViewDelegate, UITableViewDataSource  {
        @IBOutlet weak var tblViewHistoryPending: UITableView!
        @IBOutlet weak var WHSegmentedControl: UISegmentedControl!
        
        override func viewDidLoad() {
            super.viewDidLoad()
            self.tblViewHistoryPending?.delegate = self
            self.tblViewHistoryPending?.dataSource = self
            self.getHistoryAll()
        }
        ...
        func getHistoryAll() {
            ...
            self.tblViewHistoryPending?.reloadData()
        }
        
        @IBAction func SegControlChange() {
            let cell = self.tblViewHistoryPending.dequeueReusableCell(withIdentifier: "WHcell4") as! workHistoryCell
        ...
        func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            let cell = self.tblViewHistoryPending.dequeueReusableCell(withIdentifier: "WHcell4", for: indexPath) as! workHistoryCell
            cell.HDescription?.text = ""
            cell.HLocation?.text = ""
            cell.HDate?.text = ""
            cell.HStasus?.text = ""
